...
def main() -> None:
    st.set_page_config(page_title="Chilled Pump Detail", page_icon="🌬️", layout="wide")
    ...
    st.markdown(
        """
        <div class="section-title" style="font-size: 1.8rem; margin-bottom: 4px;">🌬️ Chilled Pump Systems</div>
    ...
    detail_raw = get_time_series(raw_df, equipment_type=config.EQUIPMENT_CHILLED_PUMP)
    ...
    render_power_trend_chart(detail_raw, "Chilled Pump Performance Trend", height=285)

    st.markdown("<div style='margin-bottom: 8px;'></div>", unsafe_allow_html=True)

    render_dataframe_section(
        "Chilled Pump Fleet - Operating Parameters",
        detail_latest[[
            "building",
            "unit_id",
            "power_kw",
