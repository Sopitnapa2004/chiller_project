"""
Chilled Pump Detail Page - Chiller System Dashboard
Detailed monitoring and analysis for the Chilled Pump equipment.
"""
...
def main() -> None:
    ...
    st.title("Chilled Pump Unit - Detailed Analysis")
    st.write("📊 In-depth monitoring and performance metrics for the Chilled Pump")
    ...
    with top1:
        with st.container(border=True):
            st.image(config.CHILLED_PUMP_IMAGE_PATH, use_container_width=True)
    
    with top2:
        with st.container(border=True):
            pump_kw = latest.get("chilled_pump_kw", 0)
            efficiency = _safe_efficiency(config.CHILLED_PUMP_BASELINE_TON, pump_kw)
            
            st.write(f"**Status:** ✅ Normal")
            st.write(f"**Current Power:** {pump_kw:.1f} kW")
            st.write(f"**Efficiency:** {efficiency:.1f}%")
            st.write(f"**Baseline Capacity:** {config.CHILLED_PUMP_BASELINE_TON} Ton")
    ...
    with st.container(border=True):
        ...
        if "chilled_pump_kw" in df.columns:
