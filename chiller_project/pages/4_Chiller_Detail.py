"""
Chiller Detail Page - Chiller System Dashboard
Detailed monitoring and analysis for the Chiller equipment.
"""
...
def main() -> None:
    ...
    st.title("Chiller Unit - Detailed Analysis")
    st.write("📊 In-depth monitoring and performance metrics for the Chiller")
    ...
    with top1:
        with st.container(border=True):
            st.image(config.CHILLER_IMAGE_PATH, use_container_width=True)
    
    with top2:
        with st.container(border=True):
            chiller_kw = latest.get("chiller_kw", 0)
            efficiency = _safe_efficiency(config.CHILLER_BASELINE_TON, chiller_kw)
            
            st.write(f"**Status:** ✅ Normal")
            st.write(f"**Current Power:** {chiller_kw:.1f} kW")
            st.write(f"**Efficiency:** {efficiency:.1f}%")
            st.write(f"**Baseline Capacity:** {config.CHILLER_BASELINE_TON} Ton")
